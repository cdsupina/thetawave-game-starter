    ...
    if behaviors.is_empty() {
        ...
        for (i, behavior) in behaviors.iter().enumerate() {
            ...
            let action_type_str = behavior_table
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("Unknown");
            let action_type: Option<MobBehaviorVariant> = action_type_str
                .parse()
                .map_err(|e| {
    ...
    if nested_behaviors.is_empty() {
        ...
        for (j, nested) in nested_behaviors.iter().enumerate() {
            ...
            let nested_action_str = nested_table
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("Unknown");
            let nested_action: Option<MobBehaviorVariant> = nested_action_str
                .parse()
                .map_err(|e| {
                    bevy::log::debug!(
                        "Unknown nested behavior action type '{}': {}",
                        nested_action_str,
                        e
                    );
                })
