...
pub fn build_behavior_tree(node: &BehaviorNodeData) -> Tree<Behave> {
    match node {
        BehaviorNodeData::Forever { children } => build_control_node("Forever", children),

        BehaviorNodeData::Sequence { children } => build_control_node("Sequence", children),

        BehaviorNodeData::Fallback { children } => build_control_node("Fallback", children),

        BehaviorNodeData::While {
            child,
        ...
        BehaviorNodeData::IfThen {
            condition: _condition,
            then_child,
            else_child: _else_child,
        } => {
        ...
        BehaviorNodeData::Action { name, behaviors } => {
            let component = MobBehaviorComponent {
